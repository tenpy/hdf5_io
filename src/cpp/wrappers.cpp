...
init_dispatch_tables(py::module_& m)
{
    ...
    auto add_save = [&](py::object ty, py::object method, char const* repr) {
        dispatch_save[ty] = py::make_tuple(method, py::str(repr));
    };
    auto add_load = [&](char const* repr, py::object method, py::object info) {
        dispatch_load[py::str(repr)] = py::make_tuple(method, info);
    };

    add_save(py::type::of(py::none()), saver_cls.attr("save_none"), REPR_NONE);
    ...
    auto add_dataset = [&](py::object ty, char const* repr) {
        ...
        add_save(ty, saver_cls.attr("save_dataset"), repr);
        add_load(repr, loader_cls.attr("load_dataset"), ty);
    };
    ...
    add_save(builtins.attr("dict"), saver_cls.attr("save_dict"), REPR_DICT_GENERAL);
    add_save(builtins.attr("range"), saver_cls.attr("save_range"), REPR_RANGE);
    ...
    add_save(m.attr("Hdf5Ignored"), saver_cls.attr("save_ignored"), REPR_IGNORED);
    add_save(types.attr("FunctionType"), saver_cls.attr("save_global"), REPR_FUNCTION);
    add_save(types.attr("BuiltinFunctionType"), saver_cls.attr("save_global"), REPR_FUNCTION);
    add_save(builtins.attr("type"), saver_cls.attr("save_global"), REPR_CLASS);
    py::object pybind11_type = builtins.attr("type")(saver_cls);
    if (!pybind11_type.is(builtins.attr("type")))
        add_save(pybind11_type, saver_cls.attr("save_global"), REPR_CLASS);

    add_load(REPR_NONE, loader_cls.attr("load_none"), py::none());
    add_load(REPR_STR, loader_cls.attr("load_str"), builtins.attr("str"));
    add_load(REPR_INT_AS_STR, loader_cls.attr("load_converted_to_str"), builtins.attr("int"));
    add_load(REPR_MASKED_ARRAY, loader_cls.attr("load_masked_array"), py::str(REPR_MASKED_ARRAY));
    add_load(REPR_LIST, loader_cls.attr("load_list"), py::str(REPR_LIST));
    add_load(REPR_SET, loader_cls.attr("load_set"), py::str(REPR_SET));
    add_load(REPR_TUPLE, loader_cls.attr("load_tuple"), py::str(REPR_TUPLE));
    add_load(REPR_DICT_GENERAL, loader_cls.attr("load_general_dict"), py::str(REPR_DICT_GENERAL));
    ...
    add_load(REPR_GLOBAL, loader_cls.attr("load_global"), py::str(REPR_GLOBAL));
    add_load(REPR_REDUCE, loader_cls.attr("load_reduce"), py::str(REPR_REDUCE));
