...
class Hdf5Saver:
    ...
    def save_range(self, obj, path, type_repr):
        ...
        self.save(start, subpath + 'start')
        self.save(stop, subpath + 'stop')
        self.save(step, subpath + 'step')
        return h5gr
    ...
    def save_class_or_function(self, obj, path, type_repr):
        """Save a function."""
        full_name = obj.__name__ + " in " + obj.__module__
        self.h5group[path] = full_name  # save as string dataset
        h5gr = self.h5group[path]
        h5gr.attrs[ATTR_TYPE] = type_repr
        h5gr.attrs[ATTR_CLASS] = obj.__name__  # preferably __qualname,
        # but that doesn't exist in python 2.7
